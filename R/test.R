...
taskschedulerAddinx <- function(RscriptRepository, debug = TRUE) {
  ...
  # Input.
  # For renderUI, user uiOuput.  For renderText, use textOutput.
  ui <- fluidPage(
      titlePanel("taskscheduleR"),
      fluidRow(
        column(3, wellPanel(
          textInput('rscript_repository', label = "Specify a repository to store task logs.", value = RscriptRepository), 
          textInput('taskname', label = "Choose a name for your task.", placeholder = "task_name"),
          radioButtons('frequency', label = "Schedule:", choices = c('Once', 'Every minute', 'Hourly', 'Daily', 'Weekly', 'Monthly', 'On log-on', 'On idle')),
          dateInput('startdate', label = "Start date:", startview = "month", weekstart = 1, min = Sys.Date(), format = getDateFormat(datefmt)),
          timeInput('hour', label = "Start time (24-hr):", value = Sys.time() + 3600, seconds = FALSE),
          textInput('rscript_args', label = "Additional arguments to your R script:", value = ""),
          selectInput('date_fmt', label = "Date format of your device:", choices = c("%d/%m/%Y", "%m/%d/%Y", "%Y/%m/%d", "%Y/%d/%m", "%d/%Y/%m", "%m/%Y/%d"), multiple = FALSE, selected = datefmt),                                                         
